...
class DnsServiceServicer(dns_pb2_grpc.DnsServiceServicer):
    ...
    def lookup_cds(
        ...
        if record_name == DNSLabel("@"):
            if zone.cds_disable:
                ...
                tag = tasks.make_key_tag(pub_key, flags=257)
    ...
    def lookup_reverse_ns(
        ...
        for record in models.ReverseNSRecord.objects.filter(zone=zone):
            if query_name == tasks.network_to_apra(record.network):
                found = True
                dns_res.add_answer(
                    dnslib.RR(
                        query_name,
        ...
        if query_name == tasks.network_to_apra(zone.network):
            found = True
            for ns in NAMESERVERS:
                dns_res.add_answer(
                    dnslib.RR(query_name, QTYPE.NS, rdata=dnslib.NS(ns), ttl=86400, )
    ...
    def sign_rrset(
        ...
        if is_dnssec:
            def sign_section(section, add_fn, priv_key, sign_ns=False):
                ...
                key_tag = tasks.make_key_tag(pub_key, flags=flags)
                ...
                for rtype, rrs in rtypes.items():
                    ...
                    for label, rrs in rrs.items():
                        if label.matchSuffix(DNSLabel(zone_root)):
                            ...
                            this_key_tag = tasks.make_key_tag(this_pub_key, flags=flags)
    ...
    def handle_query(self, dns_req: dnslib.DNSRecord):
        ...
        if not is_rdns:
            ...
            network = tasks.network_to_apra(zone.network)
